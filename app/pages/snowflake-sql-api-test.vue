<template>
  <div class="min-h-screen bg-gray-50 p-8">
    <div class="max-w-4xl mx-auto">
      <h1 class="text-3xl font-bold mb-6">Snowflake SQL API Test</h1>
      
      <div v-if="loading" class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
        <p class="text-blue-800">Loading employee data from Snowflake SQL API...</p>
      </div>
...
const fetchEmployee = async () => {
  ...
  try {
    const response = await $fetch<{ success: boolean; data?: any; error?: string }>('/.netlify/functions/employee-sql-api');
    
    if (response.success && response.data) {
      employee.value = response.data;
    } else {
      error.value = response.error || 'No employee data returned from API';
    }
  } catch (err: any) {
